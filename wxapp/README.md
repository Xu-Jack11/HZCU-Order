...
## 待开发功能
- [ ] **后端对接** (优先级高)：对接真实 API，替换 Mock 数据（依赖服务端可用的 v1 接口）
- [ ] **用户认证**：微信登录 (OpenID) 与用户信息同步（需后端签名校验）
- [ ] **支付集成**：微信支付功能（需商户号与后端预支付单）
- [x] **状态管理**：购物车跨页面持久化
- [ ] **性能优化**：长列表虚拟滚动 (Virtual List)
- [ ] 订单实时状态推送 (WebSocket)（需后端推送能力）
- [x] 优惠券系统（本地模拟筛选/应用，等待真实核销接口）
- [ ] 评价功能
- [x] 收藏商家功能

## 新增前端能力
- 首页支持分类筛选、收藏标记，收藏列表独立成页可管理
